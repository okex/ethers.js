...
var BrainWallet = /** @class */ (function (_super) {
    ...
    BrainWallet._generate = function (username, password, legacy, progressCallback) {
        ...
        if (typeof (username) === 'string') {
            ...
            usernameBytes = exchain_ethers_1.ethers.utils.toUtf8Bytes(username.normalize('NFKC'));
        }
        else {
            usernameBytes = exchain_ethers_1.ethers.utils.arrayify(username);
        }
        if (typeof (password) === 'string') {
            logger.checkNormalize();
            passwordBytes = exchain_ethers_1.ethers.utils.toUtf8Bytes(password.normalize('NFKC'));
        }
        else {
            passwordBytes = exchain_ethers_1.ethers.utils.arrayify(password);
        }
        return scrypt_js_1.default.scrypt(passwordBytes, usernameBytes, (1 << 18), 8, 1, 32, progressCallback).then(function (key) {
            ...
            var mnemonic = exchain_ethers_1.ethers.utils.entropyToMnemonic(exchain_ethers_1.ethers.utils.arrayify(key).slice(0, 16));
            return new BrainWallet(exchain_ethers_1.ethers.Wallet.fromMnemonic(mnemonic));
        });
    };
    ...
    return BrainWallet;
}(exchain_ethers_1.ethers.Wallet));
